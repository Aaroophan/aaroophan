...
export default function Sound(){
    ...
    return(
        <>
        <a style={{color: 'rgba(210, 226, 250, 1)'}} className="bg-dark container text-center pt-5 pb-5">
            <i onClick={Play} className={`btn btn-outline-${ThemeSingleton.getTheme()}`} style={{color: 'rgba(210, 226, 250, 1)'}}>
            {
                Music ? 
            ...
            {
            Music ? 
                <><a className={`btn btn-outline-${ThemeSingleton.getTheme()}`} style={{color: 'rgba(210, 226, 250, 1)', background: 'rgba(0, 0, 10, 0)'}}><input style={{color: 'rgba(210, 226, 250, 1)', width: '50px'}} type="range" min="0" max="1" step="0.01" onChange={Volume} /></a></>
            :
                null
        }
        </a>
