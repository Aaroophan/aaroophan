...
const ExperienceSlider = () => {
    ...
    const Description = ServerURL.Data().Experiences.map((Experience) => {
        return Experience.Description.map((Line, index) => (
            <li>{Line}</li>
        ));
    });
    
    const Slides = ServerURL.Data().Experiences.map((Experience, index) => (
        <div ref={ref[index]} className="row gx-3 text-center justify-content-center m-2">    
            <div id="LoginHere" className="col-lg-3 text-white">
                <img src={Experience.Image} height="200px" width="200px" class={`boxshadow floating zoom rounded-1 m-2 border border-${ThemeSingleton.getTheme()} border-2 rounded-5`}/><br/><br/>
            </div>
            <div className=" col-lg-8 " style={{backdropFilter: 'blur(5px) brightness(70%)', padding: '1rem', borderRadius:'25px'}}>
                <div className="card  text-white " style={{ background: 'rgba(0, 0, 0, 0)', border: 'none', display: 'flex', alignItems: 'center' }} id="Box">
                    <h2 class="zoom textshadow fw-bold mb-4" style={{ cursor:'default'}}>{Experience.Title.split("").map((letter, index) => (
                                <motion.span
                                    key={index}
                                    initial={{ opacity: 0 }}
                    ...
                    <h4 class="fw-bold mb-4" style={{ cursor:'default'}}>{Experience.Company} · {Experience.JobType}</h4>
                    <h5 class=" mb-4" style={{ cursor:'default'}}>{Experience.Location} · {Experience.LocationType}</h5>
                    <h5 class=" mb-4" style={{ cursor:'default'}}>{Experience.Date}</h5>
